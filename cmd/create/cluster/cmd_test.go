...
var _ = Describe("Validate build command", func() {
	...
	Context("build command", func() {
		When("domain prefix is present", func() {
			It("prints --domain-prefix", ci.High, func() {
...
var _ = Describe("Validates OCP version", func() {
	...
	var _ = Context("when creating a hosted cluster", func() {
		...
		It("OK: Validates successfully a cluster for hosted clusters with a supported version", ci.High, func() {
		...
		It("OK: Validates successfully a nightly version of OCP for hosted clusters "+
			"with a supported version", ci.High, func() {
		...
		cluster when the user provides an unsupported version`, ci.High, func() {
		...
		when the user provides an invalid or malformed version`, ci.High, func() {
		...
		It(`OK: Validates a supported Hypershift version successfully`, ci.High, func() {
			v, err := client.ValidateHypershiftVersion("4.14.5", stable)
			Expect(err).ToNot(HaveOccurred())
			Expect(v).To(BeTrue())
		})

		It(`KO: Fails to validate Hypershift version when the version is less than the minimal supported version`,
			ci.High, func() {
	...
	var _ = Context("when creating a classic cluster", ci.High, func() {
...
var _ = Describe("getMachinePoolRootDisk()", func() {
	...
	It("OK: isHostedCP = true", ci.High, func() {
...
var _ = Describe("getInitialValidSubnets()", func() {
	...
	It("OK: get valid subnets", ci.High, func() {
