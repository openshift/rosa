...
func run(cmd *cobra.Command, argv []string) {
	...
	if args.classic && !args.hostedCP {
		...
		_, err = roles.ValidateSharedVpcInputs(args.vpcEndpointRoleArn, args.route53RoleArn,
...
func runWithRuntime(r *rosa.Runtime, cmd *cobra.Command, preflight ...func()) error {
	...
	if err != nil {
		return fmt.Errorf("failed to retrieve account role policies: %w", err)
	}
