...
func initCreator(managedPolicies bool, hostedCP bool) creator {
	// TODO: change to `hcpManagedPoliciesCreator` once AWS Managed Policies are published
	if hostedCP {
		return &hcpUnmanagedPoliciesCreator{}
	}
...
func (up *unmanagedPoliciesCreator) createRoles(r *rosa.Runtime, input *accountRolesCreationInput) error {
	for file, role := range aws.AccountRoles {
		err := createRoleUnmanagedPolicies(r, input, file, role)
		if err != nil {
			return err
		}
	}
...
func (up *unmanagedPoliciesCreator) buildCommands(input *accountRolesCreationInput) (string, error) {
	...
	for file, role := range aws.AccountRoles {
		...
		createPolicy := buildCreatePolicyCommand(policyName, file, iamTags, input)
...
type hcpUnmanagedPoliciesCreator struct{}

func (hup *hcpUnmanagedPoliciesCreator) createRoles(r *rosa.Runtime, input *accountRolesCreationInput) error {
	for file, role := range aws.HCPAccountRoles {
		err := createRoleUnmanagedPolicies(r, input, file, role)
		if err != nil {
			return err
...
func (hup *hcpUnmanagedPoliciesCreator) getRoleTags(roleType string,
	input *accountRolesCreationInput) map[string]string {
	return getBaseRoleTags(roleType, input)
}

func (hup *hcpUnmanagedPoliciesCreator) buildCommands(input *accountRolesCreationInput) (string, error) {
	commands := []string{}
	for file, role := range aws.HCPAccountRoles {
		accRoleName := aws.GetRoleName(input.prefix, role.Name)
		iamTags := hup.getRoleTags(file, input)
...
func createRoleUnmanagedPolicies(r *rosa.Runtime, input *accountRolesCreationInput, file string,
	role aws.AccountRole) error {
	accRoleName := aws.GetRoleName(input.prefix, role.Name)
	assumeRolePolicy := getAssumeRolePolicy(file, input)

	r.Reporter.Debugf("Creating role '%s'", accRoleName)
	tagsList := getBaseRoleTags(file, input)
	roleARN, err := r.AWSClient.EnsureRole(accRoleName, assumeRolePolicy, input.permissionsBoundary,
...
func buildCreatePolicyCommand(policyName string, file string, iamTags map[string]string,
	input *accountRolesCreationInput) string {
	return awscb.NewIAMCommandBuilder().
