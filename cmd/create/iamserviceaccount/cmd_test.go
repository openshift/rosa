...
var _ = Describe("Create IAM Service Account", func() {
	...
	Describe("CreateIamServiceAccountRunner", func() {
		Context("with valid cluster", func() {
			...
			It("should classify a malformed policy ARN as a validation error and show usage", func() {
				cluster := test.MockCluster(func(c *cmv1.ClusterBuilder) {
					c.ID("test-cluster-id")
					c.Name("test-cluster")
					c.AWS(cmv1.NewAWS().
				...
				// No GetOpenIDConnectProviderByOidcEndpointUrl mock: the
				// malformed ARN must be caught by the CLI's fail-fast check
				// before that (AWS API) call ever happens.
			...
			It("should fail fast on multiple service accounts without a role name, before the OIDC lookup", func() {
				...
				// No GetOpenIDConnectProviderByOidcEndpointUrl mock: the
				// missing role name must be caught by the CLI's fail-fast
				// check before that (AWS API) call ever happens.
