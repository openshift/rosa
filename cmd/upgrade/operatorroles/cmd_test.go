...
var _ = Describe("Upgrade operator-roles", func() {
	...
	Context("handleModeFlag", func() {
		It("returns auto mode when mode is set to auto", func() {
			interactive.SetModeKey("auto")
			interactive.SetEnabled(false)
			cmd := &cobra.Command{Use: "test"}
			interactive.AddModeFlag(cmd)
			cmd.Flags().Set("mode", "auto")

			resultMode, err := handleModeFlag(cmd, "auto")
			Expect(err).NotTo(HaveOccurred())
			Expect(resultMode).To(Equal("auto"))
		})

		It("returns manual mode when mode is set to manual", func() {
			interactive.SetModeKey("manual")
			interactive.SetEnabled(false)
			cmd := &cobra.Command{Use: "test"}
			interactive.AddModeFlag(cmd)
			cmd.Flags().Set("mode", "manual")

			resultMode, err := handleModeFlag(cmd, "manual")
			Expect(err).NotTo(HaveOccurred())
			Expect(resultMode).To(Equal("manual"))
		})
	})
})
