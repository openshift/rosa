...
var Cmd = &cobra.Command{
	...
	Short:   "Grant user access to cluster specification in OCM",
	Long:    "Grant user access to view/edit cluster specification in OCM",
	Example: `  # Add role-bindings to a user
  rosa grant role-binding --user=myusername --cluster=mycluster

  # Grant ClusterEditor role to a user
  rosa grant role-binding ClusterEditor --user=myusername --cluster=mycluster`,
	Run: run,
...
func run(_ *cobra.Command, argv []string) {
	...
	reporter.Debugf("Adding role binding '%s' to user  '%s' in cluster '%s'", roleID, username, clusterKey)
	_, err = ocmClient.CreateRoleBinding(cluster.Subscription().ID(), username, roleID)
	if err != nil {
	...
	reporter.Infof("Granted role binding '%s' to user '%s' on cluster '%s'", roleID, username, clusterKey)
}
