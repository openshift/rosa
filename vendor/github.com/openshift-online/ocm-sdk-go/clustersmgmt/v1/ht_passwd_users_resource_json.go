...
func readHTPasswdUsersAddRequest(request *HTPasswdUsersAddServerRequest, r *http.Request) error {
	var err error
...
func writeHTPasswdUsersAddResponse(response *HTPasswdUsersAddServerResponse, w http.ResponseWriter) error {
	return MarshalHTPasswdUser(response.body, w)
}
func readHTPasswdUsersListRequest(request *HTPasswdUsersListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeHTPasswdUsersListResponse(response *HTPasswdUsersListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(HTPasswdUserListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeHTPasswdUserList(response.items.items, stream)
			count++
		}
	}
