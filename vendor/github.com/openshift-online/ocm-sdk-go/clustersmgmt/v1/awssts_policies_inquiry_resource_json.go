...
func readAWSSTSPoliciesInquiryListRequest(request *AWSSTSPoliciesInquiryListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeAWSSTSPoliciesInquiryListResponse(response *AWSSTSPoliciesInquiryListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(AWSSTSPolicyListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeAWSSTSPolicyList(response.items.items, stream)
			count++
		}
	}
