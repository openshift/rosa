...
func NewRosaVersion() (RosaVersion, error) {
	...
	r, err := NewRetriever(RetrieverSpec{
		Logger: logger,
	...
	if err != nil {
		return &rosaVersion{}, fmt.Errorf("failed to create retriever: %v", err)
	}
...
func (v rosaVersion) IsLatest(latestVersion string) (*goVer.Version, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("failed to retrive current version: %s", err)
	}

	latestVersionFromMirror, err := v.retriever.RetrieveLatestVersionFromMirror()
	if err != nil {
		return nil, false, fmt.Errorf("failed to retrieve latest version from mirror: %s", err)
	}
