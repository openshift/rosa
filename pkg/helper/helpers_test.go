...
var _ = Describe("Helper", func() {
	var _ = Describe("Validates Random Label function", func() {
		...
		var _ = Context("Validates ChunkSlice", func() {
			It("Returns empty when empty", func() {
				chunks := ChunkSlice([]string{}, 5)
				Expect(chunks).To(BeEmpty())
			})

			It("Returns single chunk up to size", func() {
				chunks := ChunkSlice([]string{"dremove", "any", "something"}, 5)
				Expect(chunks).To(HaveLen(1))
				Expect(chunks[0]).To(HaveLen(3))
			})

			It("Returns multiple chunks of size", func() {
				slice := []string{"test", "any", "something", "remove", "fifth"}
				chunks := ChunkSlice(slice, 2)
