...
func ValidateAutoNodeValue(value string) error {
	if value != AutoNodeModeEnabled {
		return fmt.Errorf("invalid value for --autonode. Currently only '%s' is supported", AutoNodeModeEnabled)
	}
	return nil
}
...
func ValidateRoleARN(roleArn string) error {
	...
	if !aws.RoleArnRE.MatchString(roleArn) {
		return fmt.Errorf(
			"invalid IAM role ARN format: '%s'. Expected format: arn:aws:iam::<account-id>:role/<role-name>",
			roleArn)
	}
	return nil
}
...
func ValidateAutoNodeConfiguration(autoNodeChanged, roleArnChanged, currentEnabled bool, roleArnValue string) error {
	...
	if autoNodeChanged && (!roleArnChanged || roleArnValue == "") {
		return fmt.Errorf("--autonode-iam-role-arn is required when enabling AutoNode")
	}
...
func SetAutoNode(r *rosa.Runtime, cmd *cobra.Command, cluster *cmv1.Cluster,
	...
	err := ValidateAutoNodeConfiguration(autoNodeChanged, roleArnChanged, currentAutoNodeEnabled, config.RoleARNFlag)
	if err != nil {
		return nil, err
	}
