...
var _ = Describe("OCMClient", func() {
	...
	Context("product-aware version helpers", func() {
		...
		It("gets available channels for the requested product", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(channels).To(HaveExactElements("candidate-5.0"))
		})
		...
		It("gets available upgrades for the requested product", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(upgrades).To(HaveExactElements("5.1.0"))
		})
	})
...
var _ = Describe("Versions", Ordered, func() {
	...
	Context("when creating a HyperShift cluster", func() {
		DescribeTable("Should correctly validate the minimum version with a given channel group",
			...
			Entry("OK: When a equal version of the minimum is provided",
				func() string { return "4.12.0" },
				func() string { return DefaultChannelGroup }, true, true, nil),
