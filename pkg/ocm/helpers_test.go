...
var _ = Describe("ValidateHTTPProxy", func() {
	...
	It("rejects proxy with invalid port", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal("invalid http-proxy value: port must be between 1 and 65535"))
	})

	It("rejects proxy with invalid IP and port", func() {
		err := ValidateHTTPProxy("http://10/.0.0.161:9999999")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal("invalid http-proxy value: proxy URL should not contain a path"))
	})
...
var _ = Describe("ValidateHTTPSProxy", func() {
	...
	It("rejects proxy without scheme", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("expected https-proxy to have an https:// scheme"))
	})
	...
	It("rejects proxy with invalid port", func() {
		err := ValidateHTTPSProxy("https://proxy.example.com:99999")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal("invalid https-proxy value: port must be between 1 and 65535"))
	})

	It("rejects proxy with invalid IP and port", func() {
		err := ValidateHTTPSProxy("https://10/.0.0.161:9999999")
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(Equal("invalid https-proxy value: proxy URL should not contain a path"))
	})
