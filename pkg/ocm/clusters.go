...
func (c *Client) UpdateSubscriptionNotificationContacts(
	...
	currentContacts, err := c.fetchNotificationContacts(ctx, basePath)
	if err != nil {
		return err
	}

	current := make(map[string]string, len(currentContacts))
	for _, c := range currentContacts {
		current[c.Username] = c.ID
	}
	...
	for _, username := range deduplicated {
		if _, exists := current[username]; !exists {
			...
			if resp.Status() >= 400 {
				addErrors = append(addErrors, fmt.Sprintf("'%s': HTTP %d: %s",
...
// GetSubscriptionNotificationContacts returns the usernames of all notification contacts on the
// given subscription ordered by username, or nil if none exist.
func (c *Client) GetSubscriptionNotificationContacts(
	ctx context.Context, subscriptionID string,
) ([]string, error) {
	basePath := fmt.Sprintf("/api/accounts_mgmt/v1/subscriptions/%s/notification_contacts", subscriptionID)

	contacts, err := c.fetchNotificationContacts(ctx, basePath)
	if err != nil {
		return nil, err
	}
	if len(contacts) == 0 {
...
// fetchNotificationContacts returns all notification contacts at the given API path, paginating
// through all results.
func (c *Client) fetchNotificationContacts(ctx context.Context, path string) ([]notificationContactResponse, error) {
	const pageSize = 100
	var result []notificationContactResponse
	page := 1
	for {
		resp, err := c.ocm.Get().Path(path).
			Parameter("page", page).
