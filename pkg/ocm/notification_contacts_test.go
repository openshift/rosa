...
const notificationContactsFirstPage = "page=1&size=100"
...
var _ = Describe("NotificationContacts", func() {
	...
	Describe("UpdateSubscriptionNotificationContacts", func() {
		It("adds contacts when none exist", func() {
			apiServer.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest(http.MethodGet, notificationContactsPath, notificationContactsFirstPage),
		...
		It("continues adding remaining contacts after one fails and reports summary", func() {
			apiServer.AppendHandlers(
				...
				ghttp.CombineHandlers(
					ghttp.VerifyRequest(http.MethodPost, notificationContactsPath),
					ghttp.RespondWithJSONEncoded(http.StatusCreated, map[string]interface{}{
						"id": "acc2", "username": "good_user",
					}),
				),
			)

			err := ocmClient.UpdateSubscriptionNotificationContacts(
				context.Background(), subscriptionId, []string{"bad_user", "good_user"})
			Expect(err).To(HaveOccurred())
			...
			Expect(apiServer.ReceivedRequests()).To(HaveLen(3))
		})
		...
		It("trims whitespace and skips empty entries", func() {
			...
			err := ocmClient.UpdateSubscriptionNotificationContacts(
				context.Background(), subscriptionId, []string{" user1 ", "", "  user2", " ", "user1"})
			Expect(err).NotTo(HaveOccurred())
			Expect(apiServer.ReceivedRequests()).To(HaveLen(3))
		})
			...
			apiServer.AppendHandlers(
				...
				ghttp.CombineHandlers(
					...
					func(w http.ResponseWriter, r *http.Request) {
						body, err := io.ReadAll(r.Body)
						Expect(err).NotTo(HaveOccurred())
						Expect(string(body)).To(ContainSubstring(`"account_identifier":"user1@example.com"`))
					},
					ghttp.RespondWithJSONEncoded(http.StatusConflict, map[string]interface{}{
						"kind": "Error", "reason": "notification contact already exists",
					}),
				),
			)

			err := ocmClient.UpdateSubscriptionNotificationContacts(
				context.Background(), subscriptionId, []string{"user1", "user1@example.com"})
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("failed to add 1 of 2"))
		...
		It("still deletes removed contacts even when an add fails", func() {
			apiServer.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest(http.MethodGet, notificationContactsPath, notificationContactsFirstPage),
					ghttp.RespondWithJSONEncoded(http.StatusOK, map[string]interface{}{
	...
	Describe("GetSubscriptionNotificationContacts", func() {
		...
		It("paginates through multiple pages of contacts", func() {
			apiServer.AppendHandlers(
				ghttp.CombineHandlers(
					ghttp.VerifyRequest(http.MethodGet, notificationContactsPath, notificationContactsFirstPage),
					ghttp.RespondWithJSONEncoded(http.StatusOK, map[string]interface{}{
						"items": []interface{}{
							map[string]string{"id": "acc1", "username": "user_page1"},
						},
						"size": 100,
					}),
				),
				ghttp.CombineHandlers(
					ghttp.VerifyRequest(http.MethodGet, notificationContactsPath, "page=2&size=100"),
					ghttp.RespondWithJSONEncoded(http.StatusOK, map[string]interface{}{
						"items": []interface{}{
							map[string]string{"id": "acc2", "username": "user_page2"},
						},
						"size": 1,
					}),
				),
			...
			contacts, err := ocmClient.GetSubscriptionNotificationContacts(context.Background(), subscriptionId)
			Expect(err).NotTo(HaveOccurred())
			Expect(contacts).To(Equal([]string{"user_page1", "user_page2"}))
		})
