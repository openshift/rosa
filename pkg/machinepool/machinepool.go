...
func (m *machinePool) ListMachinePools(r *rosa.Runtime, clusterKey string, cluster *cmv1.Cluster, args ListMachinePoolArgs) error {
	...
	if output.ShouldHideEmptyColumns() {
		...
		headers, tableData = removeEmptyColumnsExceptSpecial(headers, tableData, specialColumns)
	}
...
func removeEmptyColumnsExceptSpecial(headers []string, tableData [][]string, specialColumns map[int]bool) ([]string, [][]string) {
	...
	for i, header := range headers {
		...
		// For non-special columns, only keep if they have data
		if !output.CheckIfColumnIsEmpty(i, tableData) {
			newHeaders = append(newHeaders, header)
			columnsToKeep = append(columnsToKeep, i)
		}
	}
