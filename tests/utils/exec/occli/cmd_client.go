...
func NewOCClient(kubePath ...string) *Client {
	...
	if len(kubePath) > 0 {
		...
		if err == nil {
			...
			user := values[0]
			password := values[1]
			err := LoginToKubeconfigFile(user, password, config.Test.ArtifactDir)
			if err != nil {
...
func LoginToKubeconfigFile(user string, password string, kubeconfigFilePath string) error {
	...
	f, err := os.CreateTemp(kubeconfigFilePath, "temp-")
	if err != nil {
		log.Logger.Errorf("%s", err)
		return err
	}
	...
	for retryTimes > 0 {
		_, stderr, err := RunCMD("oc login " + apiUrl + " -u " + user + " -p " + password)
		if err != nil {
