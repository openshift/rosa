...
func ExtractVPCIDFromNetworkOutput(output bytes.Buffer) (string, error) {
	outputStr := output.String()
	re := regexp.MustCompile(`vpc-[0-9a-f]+`)
	match := re.FindString(outputStr)
	if match != "" {
		return match, nil
	}
	return "", fmt.Errorf("no VPC ID found in the output")
}

func ExtractCreateStackCommand(output string) (string, error) {
	re := regexp.MustCompile(`aws cloudformation create-stack.*`)
...
Conditions:
  ...
  ExplicitAZs: !Or
    - !Condition AZ1Explicit
    - !Condition AZ2Explicit
    - !Condition AZ3Explicit
    - !Condition AZ4Explicit
  ...
  One: !Or 
    - !And
      - !Condition ExplicitAZs
      - !Condition AZ1Explicit
    - !And
      - !Condition NoExplicitAZs
      - !Condition AZ1Implicit
  Two: !Or 
    - !And
      - !Condition ExplicitAZs
      - !Condition AZ2Explicit
...
Outputs:
  ...
  PublicSubnets:
    ...
    Value: !Join [",", [
      !If [One, !Ref SubnetPublic1, !Ref "AWS::NoValue"],
      !If [Two, !Ref SubnetPublic2, !Ref "AWS::NoValue"],
      !If [Three, !Ref SubnetPublic3, !Ref "AWS::NoValue"],
      !If [Four, !Ref SubnetPublic4, !Ref "AWS::NoValue"]
    ]]
    Export:
  ...
  NatGatewayId:
    ...
    Value: !Join [",", [
      !If [One, !Ref NATGateway1, !Ref "AWS::NoValue"],
      !If [Two, !Ref NATGateway2, !Ref "AWS::NoValue"],
      !If [Three, !Ref NATGateway3, !Ref "AWS::NoValue"],
      !If [Four, !Ref NATGateway4, !Ref "AWS::NoValue"]
    ]]
    Export:
