...
The CLI layer matches it. `cmd/create/iamserviceaccount/cmd.go` is the
representative migrated path for this pattern today:
...
if err != nil {
    ...
    if errors.As(err, &validationErr) {
        // Invalid input: nothing happened. Show usage so the caller can
        // fix their command, instead of a remediation guess.
        cmd.Usage()
    }
    return err
}
...
tree of individual checks automatically, so this one call finds a match
...
does this today by showing command usage for an invalid-input error (see
above). A command could go further and add remediation text for an
operational failure the same way:
...
large payloads (full policy documents, API response bodies):
