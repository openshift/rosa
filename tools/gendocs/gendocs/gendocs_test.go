...
{{end}}`
				...
				// Create hack/clibyexample directory structure
				err = os.MkdirAll("hack/clibyexample", 0755)
				Expect(err).ToNot(HaveOccurred())
				err = os.WriteFile("hack/clibyexample/template", []byte(templateContent), 0644)
				Expect(err).ToNot(HaveOccurred())
